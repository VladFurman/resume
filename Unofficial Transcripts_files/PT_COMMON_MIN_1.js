...
init:function(){
this.browser = navigator.userAgent.toLowerCase();this.isOpera = (this.browser.toLowerCase().indexOf('opera')>=0)?true:false;this.isFF = (this.browser.toLowerCase().indexOf('firefox')>=0)?true:false;this.isNetscape = (this.browser.toLowerCase().indexOf('netscape')>=0)?true:false;this.isIE = (this.browser.toLowerCase().indexOf('msie')>=0)?true:false;this.version = navigator.appVersion.replace(/.*?MSIE (\d\.\d).*/g,'$1')/1;this.isSafari = /webkit/.test(this.browser);this.isMozilla = /mozilla/.test(this.browser) && !/(compatible|webkit)/.test(this.browser);this.isSafari2x = this.Safari2x(this.browser);this.isiPad = (this.browser.toLowerCase().indexOf('ipad')>=0)?true:false;this.isiOS5 = /os 5/i.test(this.browser);},
...
PT_Dialog.prototype = {
 ...
 var minW = 0;  var minH = 0;    if (!modObj.bMsg && !modObj.bCustResizeDone && oWin.document.body.innerHTML.indexOf("javascript:DatePrompt_win") != -1 && (w < 350 || h < 175))
 {
 minW = 400; if (h < 175)
 minH = 250;  else 
 minH = h;  }
 ...
 newWidth = theBody.scrollWidth;  var oHeader = MTop().document.getElementById("ptModHeader_" + id); oHeader.style.width = newWidth + 'px';  newWidth = newWidth - 21;  if (newWidth > 0) 
...
function openSrchRsltURL(openURL){ 
 ...
 openURL += "&sesSrchTxt="; openURL += bcUpdater.getStoredData(bcUpdater.searchText); }
 ...
 $.eventAfterAJAX(fName, eName, fldDataNames);};$.attachHandler = function(theFrame)
{
 ...
 {
 var submitStr = "submitAction_" + eventDataField.form.name + "(document.getElementById('" + rtfldEvent + "').form, '" + rtfldEvent + "');"; eval(submitStr); }
 }
 ...
 eval(submitStr);};$.updateFieldData = function(fieldObject, fieldValue)
{
 ...
 fieldObject.value = fieldValue;  sScript = "addchg_"+ fieldObject.form.name+"(fieldObject);"; eval(sScript); };$.GetFramesWithMessages = function(frames)
{
 if (frames)
 {
 for (var j=0; j < frames.length; ++j)
...
};$.updateMessageEvents = function(fldName)
{
 ...
 if (typeof p_message_data != "undefined" && p_message_data.length > 0)
 $.GenerateMessageEvents(); };$.updatePromptMsgEvent = function(lookUpName)
{ 
...
var bcUpdater = {
 ...
 sScript = 'var bIsModal = false; if (typeof(bDoModal_' + theForm.name + ') != "undefined") {bIsModal=bDoModal_' + theForm.name + ';}'
 eval(sScript); if (bIsModal) {
 return false; }
 ...
 this.setStoredData(this.isGlobalSearch, "BASIC"); this.setStoredData(this.searchText, szCrefSesSrchTxt); this.appendBcElement(bcList, szCrefSesID, szCrefSesSrchTxt); if (this.getStoredData(this.isRCService) != null && this.getStoredData(this.isRCService) == "T") {
 ...
 if (typeof bcList.children[nIdx] != "undefined" && bcList.children[nIdx] && ptUtil.isClassMember(bcList.children[nIdx], "pthnavbarcref")) {
 ...
 if (name != "PTUS_ADV_SRCH_PTUS_SRCH_BTN" && 
