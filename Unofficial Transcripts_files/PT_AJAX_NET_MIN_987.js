...
if (!this.contentType && this.form && this.method.indexOf("POST")!=-1)
 ...
 url += '&ICDoModal=1&' + this.paramsModal2; if (url.length > 2048) {
 this.params = 'ICDoModal=1&' + this.params; this.loadXMLDoc(true); }
 else
...
if (this.bCallBack || !form || !form.ICAction || !form.ICResubmit || this.name.indexOf('Resubmit') != -1) return true;try {
 ...
 {
 this.WaitingObjForm = oForm; this.WaitingICAction = strAction; this.WaitingArrParams = ArrParams; this.WaitingbAjax = bAjax; this.WaitingbPrompt = bPrompt; },
 ...
 if (eId == "#ICDataLang")
 eObj.name= encodeURIComponent(eObj.name); sTmp += ptCommonObj2.getNV(eObj); if (eId.indexOf("$hnewpers$") == -1)
...
reduceScrollareaHeight:function(id, data)
 ...
 return;   if (scrollAreObj.className == "" || scrollAreObj.className == " " || (scrollAreObj.className).indexOf("SCROLLAREA")>=0)
 {
 ...
 var ct = this.req.getResponseHeader("content-type"); var bModal = this.req.getResponseHeader('bModal'); var sCancelName = this.req.getResponseHeader('sCancelName'); this.bModal = bModal; if (ptConsole2.isActive() && !bPerf) {
 ...
 if (ct.indexOf("xml") == -1)
 alert("FATAL ERROR bModal="+bModal+" - Partial page refresh should be returned, but not."); sScript = "var win = getFirstParentWin();"; loader == null; sScript += "win.nResubmit++;win.aAction_" + this.form.name + "(this.form, 'Resubmit');"; pm.updateMessageEvents(this.name); eval(sScript); return; }
 ...
 var xmlDoc = null; var bRecSep = 0;  var bSOH = 0;  var bSUB = 0;  var bFF = 0;   if (window.ActiveXObject)
 {
 ...
 if (fieldList)
 {
 var sCleanData = ""; var sData = ""; if (ptConsole2.isActive() && bPerf)
 ptConsole2.append((new Date()).valueOf() + " partial page refresh start"); for (var flIdx=fieldList.length-1; flIdx > -1; flIdx--)
 {
 ...
 continue;   sCleanData = fieldList[flIdx].firstChild.data;  this.reduceScrollareaHeight(id,sCleanData.substr(0,200));  if ((typeof bCDATA != "undefined") && bCDATA && 
 ...
 obj.innerHTML = sCleanData;  if (ptConsole2.isActive() && bPerf)
...
DoUrlAJAX: function(sURL, scriptList) {
 ...
 this.closeModal(window.modalID); net2.OnloadScriptList = scriptList; this.finalCall(); }
...
finalCall:function()
{
 ...
 this.closeModal(); if (ptConsole2.isActive() && bPerf)
 {
 ...
 {
 var objWaiting = this.GetWaitingObject(); this.SetWaitingObject(null,"",null,false,false); var sScript = "aAction0_" + (objWaiting.v).name + "(objWaiting.v, objWaiting.w, objWaiting.x, objWaiting.y, objWaiting.z);"; eval(sScript); }
 delete (this.req);},
...
if (src) {
 ...
 oStyle = document.createElement('style'); oStyle.setAttribute("id",id); oStyle.type = 'text/css'; if (browserInfoObj2.isIE) 
...
(function(path) {
 ...
 var buffer = path.match(/ps[pc]\/(.+?)(?:_\d)*?\/(.+?)\/(.+?)\//);  var url = "/psc/" + buffer[1] + "/" + buffer[2] + "/" + buffer[3] + "/s/WEBLIB_PTBR.ISCRIPT1.FieldFormula.IScript_GET_JS?ID=PT_JQUERY_1_6_2_JS"; document.write("<scr" + "ipt id='jquery_js' " + "type='text/javascript' src='" + url + "'><\/script>");  var url = "/psc/" + buffer[1] + "/" + buffer[2] + "/" + buffer[3] + "/s/WEBLIB_APT_JSL.ISCRIPT1.FieldFormula.IScript_CommonLib"; document.write("<scr" + "ipt id='APT_common_lib' " + "type='text/javascript' src='" + url + "'><\/script>");})(window.location.pathname);