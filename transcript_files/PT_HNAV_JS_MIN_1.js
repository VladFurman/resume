var listFormat = false;var ptBaseURI = getptBaseURI();var const_keyCode = { ARRLEFT : 37,ARRUP : 38,ARRRIGHT : 39,ARRDN : 40,ENTER : 13,
 ...
 pthNav.mainUl = ptUtil.id("pthnavroot"); var favBC = ptUtil.id(pthNav.bcAncPrefix + pthNav.myFavPrefix); pthNav.addBreadcrumbEvents(pthNav.container,!pthNav.isHomepage && favBC); pthNav.addEvents(pthNav.mainUl); pthNav.initCloseBtn();  ptEvent.add(document, "keyup", pthNav.onHPKeyUp); pthNav.abn.init();  if (!pthNav.isHomepage && favBC) { pthNav.initFavMRUContent(favBC); }
 ...
 ptEvent.add(pthNav.container,"mouseover",function(){pthNav.isOverNav = true; return false;}); ptEvent.add(pthNav.container,"mouseout",function(){pthNav.isOverNav = false; return false;}); ptEvent.add(document,"click",pthNav.onDocClick);  if (browserInfoObj2.isIE) {
 if (pthNav.iframe) {
 ...
 item = (browserInfoObj2.isIE)? this.parentNode.previousSibling : this.parentNode.previousElementSibling; if (item && item.nodeName.toLowerCase() === "li") {
 ...
 pthNav.fldr.sendUniNavRequest(ajaxUrl); var respHTML =  pthNav.UniNavresponse; var newdiv = document.createElement('div'); newdiv.setAttribute('id',"dummydiv"); newdiv.setAttribute('style','none'); newdiv.innerHTML = respHTML; var folderList = newdiv.getElementsByTagName('li' ); var ancSet = newdiv.getElementsByTagName('a'); if(UniNav.rootfolder == "Y"){
 ...
 var newdiv2 = document.createElement('div'); newdiv2.setAttribute('id',"dummydiv1"); newdiv2.setAttribute('style','none'); newdiv2.innerHTML = UniNav.UniNavrespHTML; var firschild = newdiv2.getElementsByTagName("ul")[1]; var secondchild = newdiv.getElementsByTagName("ul")[1]; var firstchildhtml = firschild.innerHTML + secondchild.innerHTML; firschild.innerHTML = firstchildhtml; UniNav.UniNavrespHTML = newdiv2.innerHTML; }
 ...
 var newHref = this.href + "?SEARCH_TEXT=" + encodeURI(document.pthnavsrchform.SEARCH_TEXT.value); if (typeof document.pthnavsrchform.SEARCH_TYPE != "undefined" &&
 ...
 var folderidName = folderid2[jk].split(":")[1]; var folderNode = folderid2[jk].split(":")[0]; UniNav.folderid = folderidName; if(UniNav.rootfolder == "Y")
 ...
 issamelabel = true; else if (currBC.firstChild.innerText.replace(/^\s+|\s+$/g, '') == eBC.lastChild.innerText.replace(/^\s+|\s+$/g, '')) 
 issamelabel = true; if(currBC.firstChild.host == eBC.lastChild.firstChild.host)
 issamehost = true;  if(currBC.firstChild.pathname.substring(currBC.firstChild.pathname.indexOf('/')) == eBC.lastChild.firstChild.pathname.substring(eBC.lastChild.firstChild.pathname.indexOf('/')))
 issamepath = true;   if(!issamehost)
 if((eBC.lastChild.firstChild.pathname.split("/")[3] == currBC.firstChild.pathname.split("/")[3])||(eBC.lastChild.firstChild.pathname.split("/")[3] == currBC.getAttribute("rnode")))
 issamehost=true;  } 
 else {
 if(currBC.ownerDocument.title.replace(/^\s+|\s+$/g, '') == eBC.lastChild.textContent.replace(/^\s+|\s+$/g, ''))
 issamelabel = true;  else if (currBC.firstChild.textContent.replace(/^\s+|\s+$/g, '') == eBC.lastChild.textContent.replace(/^\s+|\s+$/g, '')) 
 ...
 if(bchidden) return;    nChildren = eBC.children.length; nIdx = 0; var breadCrumb; var eBCName;  while (nIdx < nChildren) {
 ...
 var div = document.createElement("div"); div.innerHTML = eBC.children[nIdx].outerHTML; eBCName = div.children[0];  this.bcScrollUl.appendChild(div.children[0]);  } 
 ...
 "&recentsearch=" + String(pthNav.abn.search.searchEnabled);  var currNode = bc.parentNode;  var blockedEl = bc.parentNode.parentNode.parentNode.parentNode; pthNav.addBlock(bc.parentNode,blockedEl,pthNav.loadStyle); var favMRUloader = new net2.ContentLoader(
 ajaxUrl,
 null,
 null,
 "GET",
 ...
 pthNav.mru.update(bcCref.firstChild); if (pthNav.isDashboard) { pthNav.mru.save(); }
 } 
 }
 ...
 aCref.href += UnParam; UniNavTgtMRU = UniNavTgt + UnParam;  }
 }
 ...
 aCref.href = hdrFavEl.href;   if (hdrFavEl.handle) {
 ...
 var mru = tgt.parentNode;  var isNavColl = false; if (tgt.id.indexOf(pthNav.fldrAncIdPrefix) > - 1) { return; }

 
 
 
 var mruId = typeof templateChange === "undefined" ?
 pthNav.portalObjName : pthNav.getPortalObjName(mru.id);  for (var i = 0; i < this.list.length; i++) {
 ...
 clone.lastChild.className = "pthnavcrefimg";  ptEvent.add(clone,"click",pthNav.onClickCref); ptEvent.add(clone.firstChild,"keydown",pthNav.onKeyPressFAV);  var mruNodes = ptUtil.getElemsByClass("pthnavcref",pthNav.mruRoot,"li"); try {
 ...
 var ajaxUrl = pthNav.mruURI; var params = "mrulist=" + encodeURIComponent(mruData)+ "&unmruCref=" + pthNav.unmruCref + "&unmruCrefhref=" + encodeURIComponent(pthNav.unmruCrefhref);    var isAsync = true; if (typeof(link) !== "undefined" && typeof(link.href) !== "undefined") {
 ...
 1,0,null,isAsync); mruLoader = null; }
 ...
 ajaxUrl = pthNav.fldr.UniNavSwitchNode(ajaxUrl, UniNav.RNode); ajaxUrl = String(ajaxUrl).replace('\/psc\/','\/psp\/');  ajaxUrl = ajaxUrl + "&pt_fname=" + UniNav.folderid + fldrPath + "&mode=x" +
 ...
 UniNav.bUniNavMultiFolder = "true"; var ajaxUrl = pthNav.serviceURI; var RNodeUpdate = String(ajaxUrl).indexOf('\/'+UniNav.RNode+'\/'); if(RNodeUpdate == -1)
 ajaxUrl = pthNav.fldr.UniNavSwitchNode(ajaxUrl, UniNav.RNode); var folderidName = UniNav.folderid.split(":"); if(folderidName.length > 1) {
 UniNav.folderid = folderidName[1]; } 
 if(UniNav.rootfolder == "Y")
 UniNav.rootfldrLabel = UniNav.folderid;  ajaxUrl = String(ajaxUrl).replace('\/psc\/','\/psp\/');  ajaxUrl = ajaxUrl + "&pt_fname=" + UniNav.folderid + fldrPath + "&mode=x" +
 pthNav.templateType + "&templateid=" + pthNav.templateId + ptNav2Info.userLang + "&cmd=smartnav"; var respHTML1 = pthNav.openUniNav(ajaxUrl,fldrId,UniNav,fldrAnc); }
 }else{
 ...
 var hrefParts = anc.href.split("psp");  var hrefComp = anc.pathname.split("/c/"); var scriptPath = "s/WEBLIB_PT_NAV.ISCRIPT1.FieldFormula.IScript_PT_NAV_INFRAME?navtype=dropdown"; var pathNameParts = anc.pathname.split("/");      var nAdjFctr = 0; if ( browserInfoObj2.isIE ) {
 nAdjFctr = 1; }
 ...
 if (dPath.length > 1) {
 var oldString = dPath[0] + "_"; var newString = dPath[0] + ";;"; for (var j = 1; j < dPath.length; j++) {
 ...
 this.favs = ptUtil.id("ptiframatfcontent"); this.container = ptUtil.id("ptabnrscontainer"); this.srchAgain = ptUtil.id(this.srchAgainId); this.sForm = document.ptabn; this.prevDOM = ptUtil.id("ptabnprevresults"); this.promptImg = ptUtil.id("ptabnsp"); this.title = ptUtil.id(this.titleID); this.popup = ptUtil.id("ptpopup"); if (typeof(ptIframe) !== "undefined") {
 ...
 this.clearForm(); this.sForm.action = actionURL;   if (browserInfoObj2.isIE && this.container && this.container.style.display != "none") {
 this.container.style.display = "none";  }

 
 var srDiv;      if (browserInfoObj2.isIE && !document.documentMode) {
 ...
 try {
 jsData = JSON.parse(customSearch);  } catch(e) { alert(e); }
 } else {
 ...
 var urlNode = dList.childNodes[i].childNodes[j].childNodes[0].childNodes[k]; var url = urlNode.innerHTML; url = url.replace(/\&amp;/g,'&');  var linkNode = dList.childNodes[i].childNodes[j].childNodes[0].childNodes[0].childNodes[0];  linkNode.id = "srchRsltUrlPers$" + m; linkNode.tabIndex = tabIndex + 1; relActLink.href = "javascript:getRelatedActions(" + "'"+relActLink.id+"'" + ");"
 linkNode.setAttribute("ra", url);  relActLink.tabIndex = tabIndex + 2; linkNode.href = url;  urlNode.parentNode.removeChild(urlNode);  m += 1; }
 }
 }
 ...
 this.updateInMemList(results);   this.mrs.update(mru); if (mru == null) { 
 var bcLiEl = ptUtil.id(pthNav.bcCrefPrefix + pthNav.portalObjName); if (bcLiEl) {
 ...
 results = results.split(this.dataSplitKey); var sList = results[0];  var sData = results[1];   var par = this.prevDOM.parentNode; par.removeChild(this.prevDOM); ptUtil.appendNodeFromHTML(par,sData);  this.prevDOM = ptUtil.id("ptabnprevresults");  results = ptUtil.getElemsByClass("ptabnprev",this.prevDOM,"div"); var tbl;    for (var i = results.length - 1; i >= 0; i--) {
 ...
 if (this.clientPersist) {
 this.tokenCheck();  sessionStorage.removeItem(this.sKey);  sessionStorage.setItem(this.sKey,escape(this.getResultsFromDOM()));  }
 },
 ...
 var szMenuCrefNav = bcUpdater.getStoredData(bcUpdater.isMenuCrefNav);  sessionStorage.clear(); sessionStorage.setItem(this.tKey,ct);    if (typeof szMenuCrefNav != "undefined" && szMenuCrefNav) {
 ...
 var clone = this.promptImg.cloneNode(true); clone.id = this.promptImg.id + "_" + liEl.id; clone.className += " " + sClass;  ptEvent.add(clone,"click",this.onClickPrompt);  ptEvent.add(clone.firstChild,"click",this.onClickPrompt); ptEvent.add(clone.firstChild,"keydown",pthNav.onKeyPressBC);   var bcList = document.getElementById("pthbcUlScroll"); var bcChild = ""; var promptArray = new Array(); if (bcList && bcList.childNodes.length > 0) {
 for (var i=0; i < bcList.childNodes.length; i++){ 
 bcChild = pthNav.abn.search.getLastChild(bcList.childNodes[i]); if (bcChild != null && bcChild.id.indexOf("ptabnsp_") > -1) {
 promptArray.push(bcChild); }
 }
 ...
 ptEvent.add(clone,"click",pthNav.abn.search.onClickPrompt); ptEvent.add(clone.firstChild,"keydown",pthNav.onKeyPress); var mrsRoot = ptUtil.id("pthnavmrsroot");  var mrsNodes = ptUtil.getElemsByClass("pthnavcref",mrsRoot,"li"); if (mrsNodes.length !== 0) {
 ...
 var s = pthNav.abn.search; var waitDiv = s.waitDiv;  if (/\/h\/\?tab=/.test(location)) 
 positionHP_WAIT(waitDiv);  waitDiv.style.zIndex = 10000; waitDiv.style.display = "block"; var self = this; setTimeout(function(){
 ...
 s.win.popup.open(s.options); waitDiv.style.display = "none"; waitDiv.style.zIndex = "";  if (browserInfoObj2.isiPad && browserInfoObj2.isSafari) {
 ...
 var clone = this.promptImg.cloneNode(true); clone.id = this.promptImg.id + "_" + bcLiEl.id; clone.className += " " + this.bcPromptClass;  ptEvent.add(clone,"click",this.onClickPrompt);  ptEvent.add(clone.firstChild,"click",this.onClickPrompt); ptEvent.add(clone.firstChild,"keydown",pthNav.onKeyPressBC);  var bcList = document.getElementById("pthbcUlScroll"); var bcChild = ""; var promptArray = new Array(); if (bcList && bcList.childNodes.length > 0) {
 for (var i=0; i < bcList.childNodes.length; i++){ 
 bcChild = pthNav.abn.search.getLastChild(bcList.childNodes[i]); if (bcChild != null && bcChild.id.indexOf("ptabnsp_") > -1) {
 promptArray.push(bcChild); }
 }
 ...
 ptNav2Info.selectedId = pthNav.crefLiIdPrefix + s.currResults.objName; var tgt = ptUtil.id(s.mrs.mrsLiIdPrefix + s.currResults.objName); pthNav.iframeUpdate(urlStr,false,tgt.firstChild); } else {
 ...
 var s = pthNav.abn.search;   if ("ltr" === "rtl" && pthNav.IEquirksMode) {
 ...
 listFormat = true; }
 else listFormat = false;    if (typeof(s.currResults.firstOpen) === "undefined") {
 ...
 var s = pthNav.abn.search;  if (browserInfoObj2.isIE && s.container && s.container.style.display != "none") {
 ...
 var newTHead = dTable.tHead.cloneNode(true); hTable.deleteTHead(); hTable.appendChild(newTHead);  dTable.tHead.rows[0].className = "ptabndthidetr";  if (s.firstResize) { 
